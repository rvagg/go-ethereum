...
| **eth_getLogs** | Uses FilterCriteria with FromBlock/ToBlock<br>BlockHash mutually exclusive with from/to<br>Block numbers must use "0x" prefix | Similar, but highly optimized using bitmaps<br>Has more efficient topic filtering<br>Special handling for Bor protocol | Similar interface but translated to Filecoin events<br>Enforces maximum range limitation<br>Topics converted from Ethereum to Filecoin format | Erigon has bitmap optimization<br>Lotus has range limitation<br>Different topic handling |
...
|----------------|-------------|-------|------------------|
| **Block Number Tags** | Supports "earliest", "latest", "pending", "safe", "finalized" | Supports "earliest", "latest", "pending", but has different interpretations for "safe" and "finalized" | "safe" in Lotus means 30 epochs behind latest, "finalized" is 900 epochs behind latest (Filecoin's finality) |
...
|--------|--------------------------|-------------------|-------------------------------------|
| **eth_getBlockByNumber** | `BlockNumber` type with:<br>- Special tags: "earliest", "latest", "pending", "safe", "finalized"<br>- Numeric values **require** "0x" prefix (hex only)<br>- Rejects leading zeros (e.g., "0x01") | `EthBlockNumberOrHash` with:<br>- Special tags: "earliest", "latest", "pending", "safe", "finalized"<br>- Numeric values accept both decimal (no prefix) and hex ("0x" prefix)<br>- Accepts leading zeros | - Different interpretation of "safe" and "finalized"<br>- Lotus accepts decimal formats (go-ethereum rejects)<br>- go-ethereum rejects leading zeros (Lotus accepts)<br>- Lotus converts between Filecoin tipsets and Ethereum blocks |
